...
class ImageSliderView: UIView {
    ...
    lazy var pageNumberLabel: UILabel = {
        let label = UILabel()
        label.font = .primary(ofSize: StaticSize.size(10), weight: .regular)
    ...
    lazy var closeButton: UIButton = {
        let view = UIButton()
        view.setBackgroundImage(UIImage(named: "crossWhite"), for: .normal)
        return view
    }()
    ...
    func setUp() {
        ...
        closeButton.snp.makeConstraints({
            $0.top.equalToSuperview().offset(Global.safeAreaTop() + StaticSize.size(3))
